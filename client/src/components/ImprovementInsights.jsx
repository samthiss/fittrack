export default function ImprovementInsights({ insights }) {
  ...
  if (deficiencies.length === 0 && excesses.length === 0 && supplements.length === 0) {
    return (
      <div className="card">
        <p className="hint" style={{ margin: 0 }}>Rien à signaler — pas de carence ni de dépassement sur la période.</p>
      </div>
    );
  }
  ...
  return (
    <div>
      {excesses.length > 0 && (
        <>
          <h4 className="section-label">Seuils dépassés — à diminuer</h4>
          <div className="card">
            {excesses.map((e) => (
              <div className="row" key={e.key} style={{ flexDirection: 'column', alignItems: 'stretch', gap: 4 }}>
                <div className="name">
                  <span>{e.label}</span>
                  <span className="rate" style={{ color: 'var(--danger)' }}>{Math.round(e.pct)}% du seuil</span>
                </div>
                {e.topSources.length > 0 && (
                  <p className="hint" style={{ margin: 0 }}>
                    👉 Réduis en priorité : {e.topSources.map((s) => s.label).join(', ')}.
                  </p>
                )}
              </div>
            ))}
      ...
      {deficiencies.length > 0 && (
        <>
          <h4 className="section-label">Carences — à augmenter</h4>
          <div className="card">
            {deficiencies.map((d) => (
              <div className="row" key={d.key} style={{ flexDirection: 'column', alignItems: 'stretch', gap: 4 }}>
                ...
                {d.suggestion && (
                  <p className="hint" style={{ margin: 0 }}>👉 Ajoute {d.suggestion}.</p>
                )}
              </div>
            ))}
          </div>
      ...
      {supplements.length > 0 && (
        <>
          <h4 className="section-label">Compléments à envisager</h4>
          <div className="card">
            {supplements.map((s) => (
